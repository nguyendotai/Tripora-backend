...
export class TourBookingRepository {
  ...
  async expirePending(cutoff: Date): Promise<bigint[]> {
    const candidates = await this.prisma.tourBooking.findMany({
      ...
      select: { id: true, tourId: true, departureDate: true, numberOfPeople: true },
    });
    ...
    for (const candidate of candidates) {
      const applied = await this.prisma.$transaction(async (tx) => {
        const affected = await tx.$executeRaw`
          UPDATE tour_bookings SET status = 'EXPIRED', updated_at = NOW()
        ...
        await tx.$executeRaw`
          ...
          WHERE tour_id = ${candidate.tourId} AND departure_date = ${dateStr} AND booked >= ${candidate.numberOfPeople}
        `;
        return true;
      });
